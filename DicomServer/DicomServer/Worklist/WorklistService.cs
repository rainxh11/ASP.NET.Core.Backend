...
namespace DicomServerWorkList
{
...

    {
        ...
        public WorklistService(INetworkStream stream, Encoding fallbackEncoding, ILogger log, ILogManager logmanager,
            INetworkManager network, ITranscoderManager transcoder) : base(stream, fallbackEncoding, log, logmanager,
            network, transcoder)
        {
        }
        ...
        {
            Logger.Info(
                $"Received verification request from AE {Association.CallingAE} with IP: {Association.RemoteHost}");
        ...
        public async IAsyncEnumerable<DicomCFindResponse> OnCFindRequestAsync(DicomCFindRequest request)
        {
            ...
            //else
            //{
            foreach (DicomDataset result in WorklistHandler.FilterWorklistItems(request.Dataset,
        ...
        {
            Logger.Info(
                $"Received association request from AE: {association.CallingAE} with IP: {association.RemoteHost} ");

            if (WorklistServer.AETitle != association.CalledAE)
            {
                Logger.Error(
                    $"Association with {association.CallingAE} rejected since called aet {association.CalledAE} is unknown");
        ...
        public async Task<DicomNCreateResponse> OnNCreateRequestAsync(DicomNCreateRequest request)
        {
            ...
            var procedureStepId = request.Dataset
                .GetSingleValue<string>(DicomTag.StudyID).PadLeft(6, '0');
            /*.FirstOrDefault()
        ...
        public async Task<DicomNSetResponse> OnNSetRequestAsync(DicomNSetRequest request)
        {
            ...
            if (status == "COMPLETED")
            {
                ...
                var doseDescription =
                    request.Dataset.GetSingleValueOrDefault(DicomTag.CommentsOnRadiationDose, string.Empty);
                ...
                foreach (var seriesDataset in request.Dataset.GetSequence(DicomTag.PerformedSeriesSequence))
                {
                    ...
                    foreach (var instanceDataset in seriesDataset.GetSequence(DicomTag.ReferencedImageSequence))
                    {
                        ...
                        var instanceUID =
                            instanceDataset.GetSingleValueOrDefault(DicomTag.ReferencedSOPInstanceUID, string.Empty);
