...
namespace FiftyLab.PrivateSchool.Controllers
{
    ...
    public class InvoiceController : Controller
    {
        ...
        [HttpGet]
        [Route("")]
        public async Task<IActionResult> GetInvoices(InvoiceType? type, CancellationToken ct, int page = 1, int limit = -1, string? sort = "CreatedOn", bool desc = true)
        {
            try
            {
        ...
        [HttpGet]
        [Route("search")]
        public async Task<IActionResult> SearchInvoices(InvoiceType? type, CancellationToken ct, string search = "", int page = 1, int limit = -1, string? sort = "CreatedOn", bool desc = true)
        {
            try
            {
                search = $"/{search}/ig";
        ...
        [HttpPost]
        [Route("")]
        public async Task<IActionResult> CreateInvoice([FromBody] IEnumerable<InvoiceModel> models, CancellationToken ct)
        {
            ...
            try
            {
                ...
                return Ok(new ResultResponse<IEnumerable<Invoice>, string>(invoices, $" '{invoices.Count}' Invoice Created"));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> PayInvoice(string id, [FromBody] PaymentModel model, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var transactions = Enumerable
                    ...
                    .Append(new Transaction(TransactionType.Discount, model.Discount.ToDouble(0), account.ToBaseAccount()))
                    .Where(x => x.Amount > 0);
                ...
                else
                {
                    ...
                    await transaction.CommitAsync();
                    return Ok(new ResultResponse<Invoice, string>(invoice, @$"Updated Invoice '{invoice.ID}' with {model.Paid.ToDouble(0)} 
                                                                            payment amount & {model.Discount.ToDouble(0)} discount amount!"));
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> CancelInvoice(string id, CancellationToken ct)
        {
            ...
            try
            {
                ...
                return Ok(new ResultResponse<Invoice, string>(update, $"Invoice '{update.ID}', with '{cancelledTransactions.ModifiedCount}' Transactions cancelled!"));
            }
            catch (Exception ex)
            {
                if (transaction.Session.IsInTransaction)
