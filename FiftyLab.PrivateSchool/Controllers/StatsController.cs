            ...
            try
            {
                ...
                var formationCount = await DB.Fluent<Invoice>()
                    .Match(new BsonDocument("$expr", new BsonDocument("$and", new BsonArray()
                    {
                        ...
                        new BsonDocument("$gte", new BsonArray{ "$CreatedOn", new BsonDateTime(DateTime.Parse(start)) }),
                ...
                var invoiceTransaction = await DB.Database(_config["MongoDb:DatabaseName"])
                    ...
                    .Match(new BsonDocument("$expr", new BsonDocument("$and", new BsonArray()
                    {
                        ...
                        new BsonDocument("$gte", new BsonArray{ "$ChildID.CreatedOn", new BsonDateTime(DateTime.Parse(start)) }),
                        new BsonDocument("$lte", new BsonArray{ "$ChildID.CreatedOn", new BsonDateTime(DateTime.Parse(end)) }),
                    })))
                    .Group(new BsonDocument
                    {
                        {
