...
public class DatabaseChangeService : IHostedService
{
    ...
    public void StartObservables(CancellationToken ct = default)
    {
        ...
        _invoiceSubscription = invoiceChanges
            ...
            .Do(x => _backgroundJobs.Enqueue(() => _notificationSerivce.SendAll("refresh_invoice")))
            .Subscribe();
        //----------------------------------------------------------------------------------//
        ...
        _studentSubscription = studentChanges
            ...
            .Do(x => _backgroundJobs.Enqueue(() => _notificationSerivce.SendAll("refresh_student")))
            .Subscribe();
        //----------------------------------------------------------------------------------//
        ...
        _formationSubscription = formationChanges
            .Sample(TimeSpan.FromSeconds(0.25))
            .Do(x => _backgroundJobs.Enqueue(() => _notificationSerivce.SendAll("refresh_formation")))
            .Subscribe();
        //----------------------------------------------------------------------------------//
        ...
        _accountSubscription = accountChanges
            .Sample(TimeSpan.FromSeconds(0.25))
            .Do(x => _backgroundJobs.Enqueue(() => _notificationSerivce.SendAll("refresh_account")))
            .Subscribe();
        //----------------------------------------------------------------------------------//
        ...
        _transactionSubscription = transactionChanges
            .Sample(TimeSpan.FromSeconds(0.25))
            .Do(x => _backgroundJobs.Enqueue(() => _notificationSerivce.SendAll("refresh_transaction")))
            .Subscribe();
    }
}