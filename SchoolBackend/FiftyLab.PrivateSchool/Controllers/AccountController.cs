...
public class AccountController : ControllerBase
{
    ...
    [HttpPost]
    [Route("logout")]
    public async Task<IActionResult> Logout([FromServices] IBlackListStore blacklist)
    {
        try
        {
    ...
    [HttpGet]
    [Route("search")]
    public async Task<IActionResult> SearchAccounts(string search = "", int page = 1, int limit = 10,
    ...
    public async Task<IActionResult> Login([FromBody] LoginModel model, CancellationToken ct)
    {
        ...
        try
        {
            if (account != null)
            {
                ...
                this.HttpContext.Response.Cookies.Append("T", token);
    ...
    [HttpGet]
    [Route("me")]
    public async Task<IActionResult> GetCurrentAccount([FromServices] IBlackListStore store)
    {
        try
        {
            var account = await _identityService.GetCurrentAccount(HttpContext);
    ...
    [HttpGet]
    [Route("{id}/avatar")]
    public async Task<IActionResult> GetAvatar([FromServices] IWebHostEnvironment webHost, string id,
    ...
    public async Task<IActionResult> CheckAccountPassword(string id, [FromBody] dynamic model)
    {
        try
        {
            var account = await DB.Find<Account>().OneAsync(id);
            var result = account.Verify(model.Password);
            return Ok(result);
        }
        catch (Exception ex)
        {
