...
public class AttachmentBasicAuthMiddleware : IMiddleware
{
    ...
    public AttachmentBasicAuthMiddleware(ILogger<AttachmentBasicAuthMiddleware> logger, ITokenService tokenService,
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        if (!context.Request.Headers["Authorization"].Any(x => x.Contains("Bearer")) &&
            (context.Request.Path.StartsWithSegments("/api/v1/reoport") ||
