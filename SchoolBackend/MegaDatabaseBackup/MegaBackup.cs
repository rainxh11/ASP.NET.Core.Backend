    ...
    public class MegaBackup
    {
        public static async Task RunBackup(BackupOptions opts)
        {
            ...
            var outputFileZpaq = Path.Combine(backupDir,
                Utils.MakeFileSystemSafe($"{opts.BackupName}_{Environment.MachineName}_{DateTime.Now.ToString("yyyy-MM")}.zpaq"));

            App.CreateBackupZPAQ(opts.DbName, outputFileZpaq, opts.Host, opts.WorkingDir);

            ProgressBar progressBar = new ProgressBar(10000, $"Uploading Database Backup: {outputFileZpaq}", new ProgressBarOptions()
            {
